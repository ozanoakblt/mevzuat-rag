...
def is_target(r):
    return r.get("doc_id") == "yonetmelik-depolama" and r.get("madde_no") == "7" and r.get("fikra_no") == "1"

question = "Müstakil elektrik depolama tesisi kurmak için asgari kurulu güç şartı nedir?"
...
for sq in sub_queries:
    ...
    for i, r in enumerate(sq_results):
        ...
        if is_target(r) and found_at is None:
            found_at = i
    print(f"  Hedef top-30'da bulundu mu: {found_at is not None} (index: {found_at})")
